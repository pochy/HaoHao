...
func NewMachineClientService(queries *db.Queries, requiredScopePrefix string) *MachineClientService {
...
func (s *MachineClientService) Create(ctx context.Context, input MachineClientInput) (MachineClient, error) {
	if s == nil || s.queries == nil {
		return MachineClient{}, fmt.Errorf("machine client service is not configured")
	}
	...
	row, err := s.queries.CreateMachineClient(ctx, db.CreateMachineClientParams{
...
func (s *MachineClientService) Update(ctx context.Context, id int64, input MachineClientInput) (MachineClient, error) {
	if s == nil || s.queries == nil {
		return MachineClient{}, fmt.Errorf("machine client service is not configured")
	}
	...
	row, err := s.queries.UpdateMachineClient(ctx, db.UpdateMachineClientParams{
		ID:               id,
		ProviderClientID: normalized.ProviderClientID,
		DisplayName:      normalized.DisplayName,
		DefaultTenantID:  int64ToPgInt8(normalized.DefaultTenantID),
...
func (s *MachineClientService) Disable(ctx context.Context, id int64) (MachineClient, error) {
	if s == nil || s.queries == nil {
		return MachineClient{}, fmt.Errorf("machine client service is not configured")
	}

	row, err := s.queries.DisableMachineClient(ctx, id)
	if errors.Is(err, pgx.ErrNoRows) {
		return MachineClient{}, ErrMachineClientNotFound
	}
	if err != nil {
...
func (s *MachineClientService) machineClientFromDB(ctx context.Context, row db.MachineClient) (MachineClient, error) {
	...
	if row.DefaultTenantID.Valid {
		tenant, err := s.queries.GetTenantByID(ctx, row.DefaultTenantID.Int64)
		if err != nil {
