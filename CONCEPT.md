...
### 4. auth の開発依存は foundation から分離する

`v0.1 Foundation` の `compose.yaml` は PostgreSQL / Redis だけを扱い、`v0.2 Auth` で必要な Zitadel は `compose.auth.yaml` に切り出す。

- foundation の起動と auth の起動を混ぜない
- ZITADEL 公式 compose が持つ PostgreSQL / Redis と衝突させない
- local auth の canonical 手順は repo-managed seed で再現できるようにする

この repo の local auth 契約は [docs/auth-local-zitadel.md](/docs/auth-local-zitadel.md) を正とする。
...
auth / session でも同じで、browser login の token 処理や Redis session 永続化の詳細は `backend/internal/service/` に閉じる。operation や middleware には lookup 結果だけを渡す。
...
- `compose.auth.yaml`: local Zitadel 専用の standalone compose。`compose.yaml` とは merge しない
