...
この PostgreSQL は、`POSTGRESQL_SERVER_URI=postgresql://haohao:haohao@localhost:5432/haohao?sslmode=disable make sqlc-load-schema` の接続先としてそのまま使えます。`sqlc Cloud` や `SQLC_AUTH_TOKEN` は不要です（方針の詳細は「[CI ガード](#ci-ガード)」）。
...
10. sqlc の query check を確認する

```bash
POSTGRESQL_SERVER_URI=postgresql://haohao:haohao@localhost:5432/haohao?sslmode=disable make sqlc-load-schema
POSTGRESQL_SERVER_URI=postgresql://haohao:haohao@localhost:5432/haohao?sslmode=disable make sqlc-check
```

`make compose-up` 済みなら、上の接続先をそのまま使えます。fresh な DB では先に `make sqlc-load-schema` を実行し、その後に `make sqlc-check` を流します。ローカルは compose の PostgreSQL、CI は GitHub Actions の service container を使います。
...
`make sqlc-load-schema` は、`POSTGRESQL_SERVER_URI` で指定した PostgreSQL に `db/schema.sql` を流し込みます。fresh な DB を前提にしてください。

`make sqlc-check` は `sqlc generate`, `sqlc compile`, `sqlc vet` をまとめて実行します。`sqlc vet` は `backend/sqlc.ci.yaml` を使い、`POSTGRESQL_SERVER_URI` で指定した PostgreSQL に対して `sqlc/db-prepare` を走らせます。
...
`sqlc verify` は project ID を要求し、この repo の local / GitHub Actions-only 方針とは合いません。**sqlc Cloud と `SQLC_AUTH_TOKEN` は使わず、`sqlc generate` + `sqlc compile` + `sqlc vet` を Makefile 経由で再現できるチェック**に寄せています。

### 構成（何が追加されたか）

- **Makefile**: `make check-generated`, `make openapi-lint`, `make sqlc-load-schema`, `make sqlc-check` を追加済み（生成物検証・OpenAPI lint・schema snapshot 反映・`sqlc generate/compile/vet` の入口）
- **`.github/workflows/generated-artifacts.yml`**: PR / push で次を実行する
  ...
  - PostgreSQL 18 の service に `make sqlc-load-schema` を実行したうえで `make sqlc-check`
...
- SQL が static check または PostgreSQL 18 上の検証で壊れていれば CI で落ちる
...
- `make check-generated`
- `make openapi-lint`
- `POSTGRESQL_SERVER_URI=... make sqlc-load-schema`
- `POSTGRESQL_SERVER_URI=... make sqlc-check`
- `cd backend && go test ./...`
- `make build-frontend`
