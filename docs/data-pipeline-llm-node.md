...
2026-05-16 時点で、LLM node を受け止めるための前提は v1 として揃っています。`confidence_gate` は `gate_status` / `gate_reason` を行データに残し、`validate` は `validation_status` / `validation_errors_json` を行データに残し、`quarantine` はそれらの status column を使って低信頼行や validation 失敗行を通常 output から分離できます。保存済み pipeline の Inspector も backend validation / preview の `outputSchemas` を列候補の正本として扱うため、LLM node を追加するときは backend output schema resolver と validation endpoint を先に更新すれば UI warning の false positive を抑えやすい状態です。
...
次の実装計画では、まず `llm_extract_fields` v1 に絞るのが妥当です。既存の `drive_file -> extract_text -> confidence_gate -> validate -> quarantine -> human_review -> output` の流れに差し込み、LLM は field 候補、confidence、evidence、reason を出すだけにします。低 confidence、validation error、根拠不足は `quarantine` または `human_review` に回し、最終 output は明示 config と deterministic validation を通した値だけを使います。
