...
- Month 3 は 2026-05-16 時点で完了扱いです。`deleteDetection=mark_deleted`、`sameValidFromPolicy=latest_ingested_wins`、Gold publish source run/output backfill migration、frontend palette / node catalog の backend step catalog contract 化まで完了しました。
...
- 保存済み pipeline では validation endpoint を missing-column warning の正本とし、validation 未取得時は Inspector の local fallback warning を表示しないようにしました。local fallback は列候補生成と pipelinePublicId がない一時状態向けに限定します。
...
- `frontend/src/components/DataPipelineInspector.vue`: node inspector。現状は `columnsForNodeOutput()` で上流列を静的推論し、設定済み列の warning 判定に使う。runtime 出力列と同期すべき重要箇所。
...
- snapshot table の運用 UI、Gold detail の source SCD2 summary / source quality summary、Gold detail から同期元 Work table / source Data Pipeline run-output への戻り導線、Data Pipeline Runs tab の output から Gold detail への link、Gold publish history row から source Data Pipeline run/output への link、publish run 作成時点の source run/output 参照保存、既存 publish run backfill は完了済み。`deleteDetection=close_current` / `mark_deleted`、`sameValidFromPolicy=reject` / `latest_ingested_wins`、composite key 履歴 drilldown、frontend palette / node catalog の backend contract 化も backend / UI / smoke まで完了済み。Month 3 は完了扱い。
