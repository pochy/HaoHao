...
- `human_review` は `createReviewItems=true` の場合に永続 review item を作成できます。Drive text / `extract_fields` / `extract_table`、Drive JSON / `schema_mapping` の低信頼 reason は review queue へ接続済みです。Drive file detail から source file に紐づく review item / pipeline run へ戻る導線も追加済みです。product extraction 側の低信頼 review 接続はまだ未完了です。
...
- `human_review` は review item / queue の入口まで完了済み。Drive text / `extract_fields` / `extract_table`、Drive JSON / `schema_mapping` の低信頼結果と Drive file detail からの review 導線も接続済み。次は product extraction 側の導線を広げる。
...
- 次は product extraction 側の低信頼結果を `confidence_gate` / `human_review` / review item queue へ接続する。
