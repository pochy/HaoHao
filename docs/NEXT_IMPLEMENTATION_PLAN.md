...
- snapshot table の運用 UI、Gold detail の source SCD2 summary、Gold detail から同期元 Work table / source Data Pipeline detail への戻り導線、Data Pipeline Runs tab の output から Gold detail への link は完了済み。次は削除検知、Gold detail の quality summary、run/output 行への深い選択リンク、または composite key 履歴 drilldown へ進む。
