...
- `human_review` は `createReviewItems=true` の場合に永続 review item を作成できます。Drive text / `extract_fields` の低信頼 reason は review queue へ接続済みですが、`extract_table`、schema mapping、Drive file detail 側の導線はまだ限定的です。
...
- `human_review` は review item / queue の入口まで完了済み。Drive text / `extract_fields` の低信頼結果も接続済み。次は `extract_table`、schema mapping、Drive file detail 側の導線を広げる。
...
- 次は `extract_table`、schema mapping、Drive file detail 側の低信頼結果を `confidence_gate` / `human_review` / review item queue へ接続する。
