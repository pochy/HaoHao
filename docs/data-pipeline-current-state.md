...
現時点の構造化 compiler では passthrough です。つまり行や列は変更せず、前段 node の結果をそのまま後段に渡します。

使う場面は、pipeline 上で「ここでデータ品質を観察したい」という意図を残したいときです。実測値を step metadata として細かく使いたい場合は、追加実装が必要です。
...
現時点の構造化 compiler では passthrough です。つまり validation rule によって行を止めたり、run を fail させたりする処理はまだここでは実行されません。

使う場面は、将来的な品質検査や UI 表示のために「この時点で検査したい条件」を graph に持たせたいときです。現状の実行結果を制御したい場合は、`transform` の `filter` や `confidence_gate` など、実際に行を変える node を使います。
...
3. `union` を追加し、複数入力の縦結合を表現できるようにする。
4. `quarantine` を追加し、失敗行や低信頼行を通常 output と分離する。
5. `route_by_condition` を追加し、文書種別や品質状態で branch を分ける。
6. `partition_filter` / `watermark_filter` を追加し、schedule / backfill / incremental run を扱いやすくする。
7. `snapshot_scd2` を追加し、マスタや status の履歴管理を可能にする。
8. `xml_extract`、`html_extract`、`text_extract` / `markdown_extract` を追加し、入力形式を広げる。
9. `email_extract`、`log_extract` / `jsonl_extract` を追加し、業務履歴や運用ログを構造化する。
10. `pivot` / `unpivot`、`window`、`drift_check` / `anomaly_check`、`hash_fingerprint` を用途別に追加する。
