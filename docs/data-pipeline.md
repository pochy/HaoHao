...
Drive product extraction item をパイプライン行へ展開します。事前に Drive OCR と product extraction job が完了している file を入力に使います。

設定:

- `sourceFileColumn`: Drive file public ID を持つ列。default は `file_public_id`。
- `includeSourceColumns`: 入力列を出力に残すか。default は `true`。
- `confidenceThreshold`: `product_extraction_status` の判定閾値。default は `0.8`。
- `maxItems`: 1 run で展開する product item の上限。default は `1000`。

出力:

- `product_extraction_item_public_id`
- `product_name`, `product_brand`, `product_model`, `product_sku`, `product_jan_code`, `product_category`
- `product_price_json`, `product_evidence_json`, `product_attributes_json`
- `product_confidence`
...
- `product_confidence` が閾値未満、または confidence が未設定の場合は `needs_review` を出力する。
- `confidence_gate.scoreColumns=["product_confidence"]` と `human_review(createReviewItems=true)` に接続すると、低信頼商品抽出結果を review queue に残せる。
