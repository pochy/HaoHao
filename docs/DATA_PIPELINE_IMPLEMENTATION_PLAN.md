...
2026-05-15 に Month 3 の graph runtime 強化として `route_by_condition` v1 と `union` v1 を追加しました。`route_by_condition` v1 は `rules` の条件を順番に評価し、`routeColumn`、既定 `route_key` に route 名を付与します。`mode=annotate` では全行を保持し、`mode=filter_route` では `route` / `selectedRoute` に一致する行だけを後続へ渡します。structured compiler と hybrid executor の両方で実行でき、run step metadata には `routeColumn`、`defaultRoute`、`mode`、`selectedRoute`、`routes`、`routeCounts` を保存します。`union` v1 は 2 本以上の upstream を列名ベースで `UNION ALL` し、不足列を空値で補完します。任意の `sourceLabelColumn` を設定すると、どの upstream node から来た行かを保持できます。Frontend は palette、Inspector、output schema 推論、Runs metadata summary に対応済みです。検証は `make smoke-data-pipeline-route` と `make smoke-data-pipeline-union` で行います。次は `partition_filter` / `watermark_filter` を実装し、schedule run と backfill を全量処理から切り離します。
