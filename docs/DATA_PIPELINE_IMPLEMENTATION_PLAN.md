...
2026-05-15 に Month 3 の最初の graph runtime 強化として `route_by_condition` v1 を追加しました。v1 は `rules` の条件を順番に評価し、`routeColumn`、既定 `route_key` に route 名を付与します。`mode=annotate` では全行を保持し、`mode=filter_route` では `route` / `selectedRoute` に一致する行だけを後続へ渡します。structured compiler と hybrid executor の両方で実行でき、run step metadata には `routeColumn`、`defaultRoute`、`mode`、`selectedRoute`、`routes`、`routeCounts` を保存します。Frontend は palette、Inspector、output schema 推論、Runs metadata summary に対応済みです。検証は `make smoke-data-pipeline-route` で行います。次は `union` v1 を実装し、複数入力の縦結合を扱えるようにします。
