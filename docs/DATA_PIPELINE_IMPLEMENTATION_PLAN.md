...
2026-05-15 に Month 3 の graph runtime 強化として `route_by_condition` v1、`union` v1、`partition_filter` v1、`watermark_filter` v1、output node の型付き output / ordering v1 を追加しました。`route_by_condition` v1 は `rules` の条件を順番に評価し、`routeColumn`、既定 `route_key` に route 名を付与します。`mode=annotate` では全行を保持し、`mode=filter_route` では `route` / `selectedRoute` に一致する行だけを後続へ渡します。structured compiler と hybrid executor の両方で実行でき、run step metadata には `routeColumn`、`defaultRoute`、`mode`、`selectedRoute`、`routes`、`routeCounts` を保存します。`union` v1 は 2 本以上の upstream を列名ベースで `UNION ALL` し、不足列を空値で補完します。任意の `sourceLabelColumn` を設定すると、どの upstream node から来た行かを保持できます。`partition_filter` v1 は `dateColumn`、`start`、`end`、任意の `partitionKey` / `partitionValue` で行を絞り込みます。`watermark_filter` v1 は `column` と固定 `watermarkValue` で新しい行だけを残します。どちらも v1 では前回成功 run の watermark 永続管理は行わず、固定値を graph config と run step metadata に残します。Output node は `columns` で上流列を選択、リネーム、`string` / `int64` / `float64` / `bool` / `date` / `datetime` に変換し、`orderBy` を最終出力列の ClickHouse `ORDER BY` として使えるようになりました。Frontend は palette、Inspector、output schema 推論、Runs metadata summary、Output 設定 UI に対応済みです。検証は `make smoke-data-pipeline-route`、`make smoke-data-pipeline-union`、`make smoke-data-pipeline-partition`、`make smoke-data-pipeline-typed-output` で行います。次は watermark の前回成功 run 連携、または `snapshot_scd2` v1 へ進みます。
