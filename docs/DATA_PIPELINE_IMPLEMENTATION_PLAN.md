...
結論は、LLM node を先に増やすのではなく、まず **Data Pipeline の品質、可観測性、説明可能性**を実装することです。`data_pipeline_run_steps.metadata` と `DataPipelineRunStepBody.metadata` を使う Month 1 の metadata 基盤、Month 2 の失敗処理、Month 3 の主要 runtime node と optional hardening は完了扱いです。snapshot / SCD2 / Gold publish の運用 UI と lineage / quality summary への接続も v1 として完了しました。さらに SCD2 の `deleteDetection=close_current` / `mark_deleted`、`sameValidFromPolicy=reject` / `latest_ingested_wins`、composite key 履歴 drilldown、SCD2 policy の Output 設定 UI、Gold publish history row から Data Pipeline run/output へ戻る deep link、publish run 作成時点の source run/output 参照永続化、既存 Gold publish run の backfill migration、frontend palette / node catalog の backend step catalog contract 化、保存済み pipeline の API schema 優先化、`validate` の行単位 `validation_status` / `validation_errors_json` と `quarantine` 連携も実装済みです。次は Month 4 の LLM node v1 に進みます。
...
2026-05-15 に Month 3 の graph runtime 強化として `route_by_condition` v1、`union` v1、`partition_filter` v1、`watermark_filter` v1、output node の型付き output / ordering v1 を追加しました。`route_by_condition` v1 は `rules` の条件を順番に評価し、`routeColumn`、既定 `route_key` に route 名を付与します。`mode=annotate` では全行を保持し、`mode=filter_route` では `route` / `selectedRoute` に一致する行だけを後続へ渡します。structured compiler と hybrid executor の両方で実行でき、run step metadata には `routeColumn`、`defaultRoute`、`mode`、`selectedRoute`、`routes`、`routeCounts` を保存します。`union` v1 は 2 本以上の upstream を列名ベースで `UNION ALL` し、不足列を空値で補完します。任意の `sourceLabelColumn` を設定すると、どの upstream node から来た行かを保持できます。`partition_filter` v1 は `dateColumn`、`start`、`end`、任意の `partitionKey` / `partitionValue` で行を絞り込みます。`watermark_filter` v1 は `column` と固定 `watermarkValue`、または前回成功 run の `nextWatermarkValue` で新しい行だけを残します。前回成功 run mode では初回は graph config の `watermarkValue` を initial value として使い、各 run の step metadata に `watermarkValue`、`resolvedSource`、`previousRunPublicId`、`nextWatermarkValue` を保存します。Output node は `columns` で上流列を選択、リネーム、`string` / `int64` / `float64` / `bool` / `date` / `datetime` に変換し、`orderBy` を最終出力列の ClickHouse `ORDER BY` として使えるようになりました。2026-05-16 には `snapshot_scd2` v1、output `writeMode=append`、output `writeMode=scd2_merge` を追加しました。`snapshot_scd2` は入力内の履歴行を `uniqueKeys` と `updatedAtColumn` で並べ、`valid_from`、`valid_to`、`is_current`、`change_hash` を付与します。`append` は既存 output table があれば stage table から追記し、なければ初回 table として作成します。`scd2_merge` は既存 snapshot output table の current row と今回 run の current row を `uniqueKeys` と `change_hash` で比較し、変更がある key だけ既存 current row の `valid_to` を新 version の `valid_from` で閉じ、新しい current row を追加します。同一データの再実行では行数を増やさないため、manual rerun / schedule rerun の idempotency を満たします。`deleteDetection=close_current` を明示した場合は、今回 run の current snapshot に存在しない既存 current key も close します。`deleteDetection=mark_deleted` では close に加えて削除マーカー列付き current tombstone row を追加します。`sameValidFromPolicy=reject` では、同一 key / 同一 `valid_from` に異なる `change_hash` がある stage を promote 前に失敗させます。`sameValidFromPolicy=latest_ingested_wins` では `ingestedAtColumn` の最新値を採用します。`scd2MergePolicy=rebuild_key_history` では、stage に含まれる key だけ既存履歴と今回履歴を結合し、`valid_to` / `is_current` を再計算します。これにより late arriving data を途中の履歴として差し込めます。Data Pipeline run output metadata には `workTablePublicId`、database、table name、display name、write mode、SCD2 merge policy、delete detection policy、same valid-from policy を保存するようになり、Runs tab から output Work table を既存 Gold publication API へ渡して Gold detail へ遷移できます。Work table preview / history は composite key に対応し、`scd2Summary.keyColumns` と `scd2-history?keyColumns=...&keyValues=...` で履歴を取得できます。Frontend は palette、Inspector、output schema 推論、Runs metadata summary、Output 設定 UI、Output からの Gold publish 導線、composite key history input、SCD2 policy 設定に対応済みです。Gold detail の publish history は各 `DatasetGoldPublishRunBody.sourceDataPipelineRun` を表示し、source pipeline の `runPublicId` / `outputNodeId` へ deep link できます。さらに `dataset_gold_publish_runs.source_data_pipeline_run_id` / `source_data_pipeline_run_output_id` に publish run 作成時点の source run/output を保存するため、後続 run が同じ Work table を更新しても新規 publish run は当時の source output へ戻れます。既存行は `0050_backfill_dataset_gold_publish_run_source_refs` で best-effort backfill され、2026-05-16 local DB では 4/4 件の補完を確認しました。frontend palette / node catalog は `/api/v1/data-pipelines/step-catalog` から取得します。保存済み pipeline の Inspector は backend validation / preview の `outputSchemas` を列候補の正本として扱い、`validate` は `validation_status` / `validation_errors_json` を行データに追加し、`quarantine` はそれを直接隔離条件に使えます。検証は `make smoke-data-pipeline-route`、`make smoke-data-pipeline-union`、`make smoke-data-pipeline-partition`、`make smoke-data-pipeline-typed-output`、`make smoke-data-pipeline-watermark-previous`、`make smoke-data-pipeline-snapshot-scd2`、`make smoke-data-pipeline-snapshot-append`、`make smoke-data-pipeline-snapshot-merge`、`make smoke-data-pipeline-snapshot-merge-delete`、`make smoke-data-pipeline-snapshot-merge-conflict`、`make smoke-data-pipeline-snapshot-merge-composite`、`make smoke-data-pipeline-snapshot-merge-backfill`、`go test ./backend/internal/service`、`npm --prefix frontend run build` で確認します。Month 3 とその直後の hardening は完了扱いで、次は Month 4 の LLM node v1 に進みます。
...
現在 backend catalog に存在する step type は多いですが、catalog にあることと、すべての実行経路で同じ深さまで実装されていることは別です。`profile` は行データを変更せず run step metadata に実測 summary を保存します。`validate` は run step metadata に validation summary を保存し、さらに ClickHouse 行データへ `validation_status` と `validation_errors_json` を追加します。
...
- `data_pipeline_compile.go` の `compileNode` では `DataPipelineStepProfile`、`DataPipelineStepValidate` は行データ上は `passThrough` です。`DataPipelineStepOutput` は `columns` が未設定なら実質 pass-through、設定済みなら最終出力列の選択、リネーム、型変換を行います。run executor 側で `profile` / `validate` metadata を収集します。
...
- `human_review`: review 対象件数、queue 名、理由 column、永続 review item を入れる。
...
`validate` は run step metadata に validation summary を保存し、行データにも `validation_status` と `validation_errors_json` を追加します。`validation_status` は `pass`、`warning`、`error` を持ち、`validation_errors_json` は失敗 rule の column、operator、severity、reason、ruleIndex を JSON array 文字列として残します。`quarantine` node は `statusColumn` 未指定時に上流の `validation_status` を検出し、`error` / `warning` を隔離対象にできます。
...
- Drive text / `extract_fields` / `extract_table`、Drive JSON / `schema_mapping`、Drive product extraction の低信頼結果は `confidence_gate` / `human_review` / review item queue へ接続済み。Drive file detail から該当 review item / pipeline run へ戻る導線も追加済み。
...
- output node の型付き output / `orderBy` 設定を追加する。
...
- 最終 Work table に必要な列だけを型付きで出力し、ClickHouse の `ORDER BY` を設定できる。
...
- structured compiler tests
  ...
  - `validate` が `validation_status` / `validation_errors_json` を行データ列として出すこと。
  - `quarantine` が `validation_status` を default status column として扱えること。
- service tests
