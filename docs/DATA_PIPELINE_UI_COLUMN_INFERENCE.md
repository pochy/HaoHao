...
- `DataPipelineInspector.vue` の node 別推論が追加された状態を維持する。
- 新しい node または出力列を増やす場合、backend materializer / compiler と Inspector 推論を同じ PR で更新する。
...
- frontend は output columns を個別 hardcode せず、generated contract または API から取得した step schema を使う。
