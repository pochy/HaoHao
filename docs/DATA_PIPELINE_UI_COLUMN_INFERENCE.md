...
結論は、backend runtime の出力列と frontend Inspector の静的列推論が別々に実装されていたことが根本原因である。直近では `DataPipelineInspector.vue` の node 別推論を backend の materializer と合わせて修正し、その後に validation endpoint / preview API が返す `outputSchemas` を優先する形へ寄せた。2026-05-16 時点では、保存済み pipeline の通常操作では backend `outputSchemas` が列候補と missing-column warning の正本である。
...
- passthrough: `profile`, `clean`, `normalize`, `output`, `quarantine`
- row annotation: `validate` は `validation_status` と `validation_errors_json` を追加する。
...
- `5810f80 Prefer API schemas in pipeline inspector` で、保存済み pipeline の列候補は validation / preview の `outputSchemas` を優先し、backend schema がない場合は手書き step 推論へ落ちないようにした。
- `e879763 Annotate validation rows for quarantine` で、`validate` node が `validation_status` / `validation_errors_json` を出力列として追加するようになった。`quarantine` は `statusColumn` 未指定時に `validation_status` があればそれを優先し、`error` / `warning` を default match values として扱う。
...
- 保存済み pipeline では validation endpoint を missing-column warning の primary source とし、validation 未取得時は Inspector の local fallback warning を表示しない。local fallback は pipelinePublicId がない一時状態向けに残す。
- 保存済み pipeline の列候補も backend `outputSchemas` を優先する。これにより、`extract_text`、`schema_mapping(includeSourceColumns=true)`、`human_review`、`product_extraction`、`snapshot_scd2`、`validate` などの runtime 出力列は、UI でも API schema を正本として扱う。
- backend の `dataPipelineStepCatalog` または近い場所に output schema metadata を持たせる。
- frontend は `data-pipeline-step-output-schema.ts` の手書き contract を new draft / endpoint 未取得時の fallback に寄せる。さらに進めるなら、この fallback module 自体を backend schema resolver から生成する。
