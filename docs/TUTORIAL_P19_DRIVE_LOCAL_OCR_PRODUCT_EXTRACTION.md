...
- outbox は `outbox_events` と `P7OutboxHandler` の event type dispatch で処理している
...
backend/internal/service/drive_phase9_service.go
backend/internal/service/outbox_handler.go
backend/internal/service/tenant_settings_service.go
backend/internal/api/drive_files.go
backend/internal/api/drive_phase9.go
...
```bash
rg -n "UploadFile|OverwriteFile|CreateAIJob|drive_search_documents|UpsertDriveSearchDocument|P7OutboxHandler|features.drive|DrivePolicy" backend db frontend/src
...
`P7OutboxHandler` に OCR service を extra dependency として受け取れるようにします。
...
- PP-OCRv5 / PP-StructureV3 helper を local 実行する
