...
- step output schema はまだ backend と frontend の単一正本ではない。Inspector の false positive は直近修正済みだが、新しい node / 出力列を増やすたびに `DataPipelineInspector.vue` の静的推論を追随させる必要がある。
