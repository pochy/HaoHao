...
- Drive OCR / product extraction の低信頼結果から review queue へ到達する画面導線は未完成。schema mapping は `schema_mapping_confidence` を `confidence_gate` / `human_review` に渡す smoke まで実装済み。
...
Status: 実装中。2026-05-14 に `extract_fields` / `extract_table` / `schema_mapping` の信頼度 metadata と、Drive text / JSON -> extraction / mapping -> `confidence_gate` -> `human_review(createReviewItems=true)` の smoke を追加した。Reviews tab では source snapshot から Drive file / OCR run の trace を表示し、Drive file detail から `source_snapshot.file_public_id` に紐づく review item / pipeline run へ戻る導線も追加済み。
...
次に実装するのは Phase 4 の product extraction 連携と、review item 権限 test の補強です。

最小完了条件:

1. product extraction の低信頼 result を `confidence_gate` / `human_review` に渡す sample graph を作る。
2. schema mapping の低信頼 review smoke を suite で維持する。
