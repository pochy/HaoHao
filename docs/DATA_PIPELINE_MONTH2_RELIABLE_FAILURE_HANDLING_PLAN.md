...
Month 2 の目的は、Data Pipeline の失敗行、低信頼行、レビュー待ち行を通常 output から分離し、理由と履歴を追える状態にすることです。Month 1 で run step metadata、`profile`、`validate`、`quality_report`、`confidence_gate` の土台は完了済みです。Month 2 の `quarantine` node v1、失敗理由 metadata 強化、`human_review` の永続 review item / queue 入口、Drive / OCR / extraction 連携は実装済みです。

この文書は Month 2 の実装を別セッションで開始するための詳細計画として作成しました。2026-05-14 時点で Month 2 の主要実装は完了済みです。現在は履歴として残し、Month 2 の判断根拠、完了条件、残課題を確認するために使います。直近の Month 3 / SCD2 / Gold publish まで含む引き継ぎは `docs/DATA_PIPELINE_SESSION_HANDOFF.md` を参照してください。
...
- `human_review` は `review_status`、`review_queue`、`review_reason_json` を行データへ追加できる。`createReviewItems=true` の場合は永続 review item を作成し、Reviews tab、transition、comment API から扱える。
- Data Pipeline smoke suite は JSON、Excel、text、quarantine、field review、product review scenario で run step metadata、quarantine output 分離、review item 作成、Drive file から review item への追跡を検証する。
- Inspector の上流列 warning は、代表的な hybrid node について runtime 出力列と静的推論を同期済み。`extract_text -> quality_report` の `text, confidence` と、`schema_mapping(includeSourceColumns=true) -> human_review -> output` の `file_public_id` false positive は修正済み。詳細は `docs/DATA_PIPELINE_UI_COLUMN_INFERENCE.md` を参照する。

残課題:

- `validate` の失敗行は metadata に集計されるが、後続 node が行単位で読める validation status 列はまだない。
- `human_review` は永続 review item / queue の入口になった。ただし担当者割当、修正値の再投入 run、修正履歴 UI はまだ薄い。
...
- step output schema は backend validation endpoint を primary source にする方向へ進んだが、backend と frontend の単一正本化はまだ完了していない。Inspector の false positive は代表ケースを修正済みだが、新しい node / 出力列を増やすたびに `DataPipelineInspector.vue` の fallback 推論も確認する必要がある。
...
2026-05-16 時点の判定: Month 2 の主要完了条件は達成済みです。残っているものは Month 2 の blocker ではなく、次フェーズで扱う review item の担当者割当、修正値の再投入 run、review 履歴 UI、step schema contract の単一正本化です。
...
次に実装するのは review item 権限 test の補強、Data Pipeline step output schema contract の単一正本化、Month 3 の graph runtime 強化です。
...
6. backend step catalog または validation / preview API に node ごとの inferred output schema を持たせ、Inspector の列推論を単一正本へ寄せる。
7. representative graph に対し、`output.orderBy`、`quality_report.columns`、`confidence_gate.scoreColumns`、`human_review.reasonColumns` が UI 推論上も存在することを自動検証する。
