...
次に実装するのは Phase 3 の `human_review` queue 化です。

最小完了条件:

1. `data_pipeline_review_items` の DB migration と sqlc query を追加する。
2. review item list / detail / transition / comment API を追加する。
3. `human_review` node に `createReviewItems` config を追加する。
4. `review_status = needs_review` の行から tenant-scoped review item を作成する。
5. Data Pipeline detail に review item 入口を追加する。
6. tenant boundary、CSRF、audit、idempotency の test を追加する。
