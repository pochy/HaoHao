...
次に実装するのは Phase 2 の失敗理由強化です。

最小完了条件:

1. `confidence_gate` に `gate_reason` 列を追加する。
2. score column 欠損、数値変換不能、threshold 未満を reason として区別する。
3. `confidence_gate` metadata に `lowConfidenceSamples` を最大 5 件保存する。
4. `quality_report` metadata に threshold 超過 warning を追加する。
5. 将来の `validate -> quarantine` 連携用に `validation_status` / `validation_errors_json` の列 contract を固める。
