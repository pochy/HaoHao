...
- ただし `POST /api/v1/login` は `501 Not Implemented` で止めている

重要なのは最後です。**足場はあるが、本物の Zitadel login はまだ通っていない**という状態を、この文書で前に進めます。
...
### Step 0.2. 公式 Docker Compose quickstart で Zitadel を起動する

self-hosted の最短導入は、Zitadel の公式 Docker Compose quickstart を使うのが安全です。ここでは repo 内に compose を持ち込まず、**公式の quickstart を別ディレクトリで起動する**前提にします。
...
#### この repo で実際に使った quickstart 配置と起動コマンド

公式 docs は `docker compose` 表記ですが、この repo の Makefile は `docker compose` と `docker-compose` の両方を許容しています。以下では docs に合わせて `docker compose` を書きます。plugin が無い環境では `docker-compose` に読み替えてください。

```bash
mkdir -p .local/zitadel-compose
cd .local/zitadel-compose
...
公式 quickstart は Zitadel を `http://localhost:8080` に公開します。しかし、この repo の HaoHao backend も `http://127.0.0.1:8080` を使います。**同じ host で両方を起動する場合は、Zitadel 側の公開 port を先にずらしてください。**

quickstart の `.env` では、少なくとも次を変更します。
...
実際には `.local/zitadel-compose/.env` を開き、元の `8080` を上の 2 行だけ `8081` に置き換えれば十分です。その後に起動します。
...
- `db/migrations/0003_user_identities.up.sql`
- `db/migrations/0003_user_identities.down.sql`
- `db/migrations/0004_roles.up.sql`
- `db/migrations/0004_roles.down.sql`
- `db/migrations/0005_downstream_grants.up.sql`
- `db/migrations/0005_downstream_grants.down.sql`
- `db/migrations/0006_scim_provisioning.up.sql`
- `db/migrations/0006_scim_provisioning.down.sql`
- `db/migrations/0007_org_tenants.up.sql`
- `db/migrations/0007_org_tenants.down.sql`
- `db/migrations/0008_machine_clients.up.sql`
- `db/migrations/0008_machine_clients.down.sql`
...
`backend-dev` と `frontend-dev` は別 terminal で起動してください。現在の repo は `frontend/vite.config.ts` で `host: '127.0.0.1'` を固定しているため、`make frontend-dev` をそのまま使えます。
...
2. Zitadel quickstart を別ディレクトリで起動する
...
curl -fsSLO https://raw.githubusercontent.com/zitadel/zitadel/main/deploy/compose/.env.example
cp .env.example .env
# .env で PROXY_HTTP_PUBLISHED_PORT=8081 と ZITADEL_EXTERNALPORT=8081 を設定
docker compose --env-file .env -f docker-compose.yml up -d --wait
cd ../..
```
...
これで token verify, auth context, OpenAPI security scheme, cross-origin 動作の全てを小さく一周できます。その後に TODO 一覧などの read API を足してください。外部向け一覧は `CONCEPT.md` どおり `cursor` pagination を優先します。
