...
ただし、P7 までで API surface が大きく増え、`openapi/openapi.yaml` の変更量もかなり大きくなっている。したがって、次に最初に着手するべきなのは **`openapi/openapi.yaml` の適切な分割** である。その後に、分割後の frontend SDK と browser flow を Playwright E2E で固めるのがよい。

### 次の優先順位

1. `openapi/openapi.yaml` の適切な分割
2. P7 UI Playwright E2E チュートリアル作成 / 実装
3. P10 チュートリアル作成
4. tenant settings の rate limit runtime 連動
5. file lifecycle の物理削除
6. P10 候補の順次実装

#### 1. `openapi/openapi.yaml` の適切な分割

最優先は、単一の巨大な `openapi/openapi.yaml` を、利用者と security boundary に沿って分割することである。

P7 までで browser session API、tenant admin、Customer Signals、file、notification、invitation、tenant settings、data export、external bearer、SCIM、M2M が同じ spec に載っている。これは生成や diff 確認には便利だが、frontend SDK、外部公開 API、machine client API、運用 docs の責務が混ざりやすい。

推奨する分割は次である。

- `openapi/openapi.yaml`: full canonical spec。CI の drift check と全体 docs の正本として残す。
- `openapi/browser.yaml`: frontend generated SDK 用。Cookie session / CSRF 前提の browser API、tenant admin、Customer Signals、file、notification、invitation、tenant settings、data export を含める。
- `openapi/external.yaml`: 外部連携用。external bearer、M2M、SCIM、delegated auth など、browser Cookie に依存しない API を含める。

この分割に合わせて、OpenAPI export command は full / browser / external を出せるようにし、frontend の `@hey-api/openapi-ts` は `browser.yaml` だけを見る形に寄せる。これにより、frontend generated client に外部 API や M2M API が混ざることを避けられる。

完了条件は次にする。

- full / browser / external の spec が生成できる。
- frontend generated SDK は browser spec 由来になる。
- CI または make target で spec drift を検知できる。
- `/openapi.yaml` の互換性をどう扱うかが決まっている。初期は full canonical を返すのがよい。

#### 2. P7 UI Playwright E2E

OpenAPI 分割の次は、P7 で増えた UI と既存の login / tenant flow を browser レベルで確認することである。
...
P10 は、P7 の上に載せる横断機能を分離して設計するのがよい。P7 初期版に詰め込むより、outbox、audit、tenant settings、file storage、metrics が入った状態で独立 tutorial として扱う方が実装と検証が安定する。
...
すぐ次に進むなら、まず **`openapi/openapi.yaml` の適切な分割** を行うのがよい。

理由は、P7 で API surface が増え、単一 spec のままでは frontend SDK と外部公開 API の境界が曖昧になり始めているからである。OpenAPI を利用者別に分割した上で、次に Login から tenant 選択、Customer Signals、file upload、Tenant Admin settings / export までを 1 本の browser E2E として通すと、P0-P7 の browser app と single binary 配信の回帰検知が一段強くなる。
