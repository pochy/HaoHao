...
Work table v1 では、Work table の source Dataset と created query job までを保存します。Phase 1-3 で export schedule、scheduled export、Dataset sync history も metadata として蓄積されます。Phase 4 では、これらの保存済み metadata を正本にして、ユーザーが「この table / Dataset は何から作られ、どこで使われたか」を確認できるようにします。
...
- Work table の scheduled export と実行履歴
- Dataset 化後の resync history

UI では、最初から複雑な graph editor にしません。Dataset detail と Work table detail に lineage section を追加し、一次関係を list / timeline と compact graph で見せます。Graph は read-only で、編集、手動 edge 追加、SQL parser による自動推定 edge は v1 に含めません。

### Summary

Phase 4 v1 は、lineage 専用の正本 table を先に作らず、既存 table の foreign key と履歴 table から read-only graph を組み立てます。重複した lineage record を保存すると、export / schedule / sync の状態と不整合になりやすいためです。

初期対象は次の metadata lineage に限定します。

| Relation | Source metadata | Edge |
| --- | --- | --- |
| Dataset -> query job | `dataset_query_jobs.dataset_id` | Dataset が query job の input になった |
| query job -> Work table | `dataset_work_tables.created_from_query_job_id` | query job が Work table を作成した |
| Dataset -> Work table | `dataset_work_tables.source_dataset_id` | Work table が Dataset を source として持つ |
| Work table -> promoted Dataset | `datasets.source_work_table_id` | Work table から Dataset 化された |
| Work table -> export | `dataset_work_table_exports.work_table_id` | Work table から export が作成された |
| schedule -> export | `dataset_work_table_exports.schedule_id` | schedule により export が作成された |
| Work table -> export schedule | `dataset_work_table_export_schedules.work_table_id` | Work table に export schedule がある |
| Work table -> sync job -> Dataset | `dataset_sync_jobs.source_work_table_id` / `dataset_sync_jobs.dataset_id` | source Work table から Dataset を full refresh した |

### DB / API

v1 では新しい lineage 正本 table は追加しません。必要に応じて、既存 table に read performance 用の index だけを追加します。index を追加する場合も、生成物を手書き編集せず migration と generator 経由で反映します。

Browser API は resource 起点の read-only endpoint とします。

- `GET /api/v1/datasets/{datasetPublicId}/lineage`
- `GET /api/v1/dataset-work-tables/{workTablePublicId}/lineage`
- `GET /api/v1/dataset-query-jobs/{queryJobPublicId}/lineage`

Query parameter は最小限にします。

- `direction`: `upstream | downstream | both`。default は `both`
- `depth`: default `1`、max `2`
- `includeHistory`: export / sync job の古い履歴を含めるか。default `true`
- `limit`: history node の上限。default `50`、max `100`

Response は UI が graph と timeline の両方を描ける形にします。
...
```json
{
  ...
  "edges": [
    {
      "id": "edge_...",
      "sourceNodeId": "dataset:ds_...",
      "targetNodeId": "query_job:dqj_...",
      "relationType": "query_input",
      "confidence": "metadata",
...
Response には内部 numeric id、ClickHouse credential、file storage path、raw SQL error detail を含めません。query job の SQL statement を返す場合は、既存の query history と同じ認可条件に合わせ、長文は UI 側で折りたたみます。

### Backend

`LineageService` を追加し、既存 metadata を横断して `LineageGraph` を組み立てます。

- root resource の tenant と閲覧権限を最初に検証する
- tenant boundary をすべての SQL query の条件に含める
- node id は `resource_type:public_id` のような stable key にして重複を排除する
- relation type は enum 相当の固定値を backend 内で定義する
- `confidence` は v1 では常に `metadata` とする
- graph traversal は depth 2 までに制限し、巨大 graph を返さない
- export / sync history は最新順で limit する
- deleted / failed / skipped の履歴も status 付きで表示できるようにする
- Work table preview / lineage read は query history に追加しない

主な query は SQLC で追加します。OpenAPI と frontend SDK は `./scripts/gen.sh` で再生成し、生成物は手書きしません。

### Frontend

Dataset detail と Work table detail に read-only lineage section を追加します。

Dataset detail では次を表示します。

- source Work table がある場合の upstream
- linked Work tables と query jobs の downstream
- promoted source / resync history
- latest sync status への導線

Work table detail では次を表示します。

- source Dataset
- created query job
- promoted Dataset
- manual / scheduled export history
- export schedule
- Dataset sync job で使われた履歴

UI は最初に次の 2 表現を提供します。

- `Lineage timeline`: 作成、promote、export、schedule run、sync を時系列で表示する
- `Lineage graph`: root を中心に upstream / downstream node を並べる compact な read-only graph

Graph は v1 では既存 Vue component と CSS / SVG で実装し、重い graph editor dependency は追加しません。node click で Dataset / Work table / export / sync job の既存 detail へ移動できる範囲だけ link します。権限のない node は public id や display name を出さず、`restricted` として件数だけ分かる扱いにします。

i18n には、lineage、upstream、downstream、source、created by query、promoted dataset、exported file、scheduled export、sync job、restricted、metadata confidence の文言を追加します。
...
SQL parser を導入する将来 Phase では、parser が作った推定 edge と metadata edge を混ぜないようにします。推定 edge には `confidence=parsed` または `confidence=inferred` を付け、parser failure は lineage API 全体の失敗にしません。

### Realtime / observability

Lineage は read-only view なので、v1 では専用 realtime event を追加しません。既存の `job.updated` により export / sync 画面が更新された後、lineage section は表示中画面の通常 refresh または手動 refresh で更新します。

必要に応じて次を log / metrics で追跡します。

- lineage endpoint の duration
- returned nodes / edges count
- limit による truncation count
- restricted node count
- lineage query error count

### Test Plan

Backend では次を確認します。

- Dataset 起点 lineage で linked Work table、query job、promoted source、sync history が返る
- Work table 起点 lineage で source Dataset、created query job、promoted Dataset、exports、schedules が返る
- query job 起点 lineage で input Dataset と created Work table が返る
- `direction`、`depth`、`limit` が期待通りに効く
- 他 tenant / 権限外 resource は返らない
- response に内部 numeric id、storage path、credential、raw error detail が含まれない
- SQL parser が未実装でも endpoint が metadata lineage を返す

Frontend では次を確認します。

- Dataset detail に lineage timeline / compact graph が表示される
- Work table detail に source / created query / promoted Dataset / export / schedule / sync が表示される
- node click が既存 detail route へ遷移する
- restricted node が情報漏えいしない表示になる
- empty lineage でも UI が崩れない
...
- Work table detail から source Dataset、created query job、promoted Dataset、exports、schedules、sync usage が分かる
- SQL parser に頼らない metadata lineage が read-only API で取れる
- parser が未実装でも lineage UI が破綻しない
- depth / limit により巨大 lineage を返しすぎない
- 権限外 resource や cross-tenant resource の情報が漏れない
- response に内部 id や file storage path を含めない
- `go test ./backend/...` と `cd frontend && npm run build` が通る
