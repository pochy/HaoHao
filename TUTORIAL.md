...
| --- | --- | --- | --- |
| 1 | Step 1-3 | `go.work`, `backend/go.mod`, `frontend/package.json`, `compose.yaml`, `Makefile`, `scripts/gen.sh` | リポジトリ全体の実行土台を作る |
...
| 4 | Step 13-19 | `frontend/vite.config.ts`, `frontend/src/*` | 生成 client を transport wrapper 経由で使う |
...
- `frontend/package.json` の一部と lockfile
...
この段階では `dotenv` ライブラリは使いません。`Makefile` 側で `.env` を読み込んで Go コマンドへ渡します。
...
1. Go + Huma から OpenAPI を出す
2. OpenAPI から TypeScript SDK を作る
3. SQL から Go コードを作る

後から人によって順番が変わるのを防ぐために、最初からスクリプト化しておきます。
...
db-up:
	migrate -path db/migrations -database "postgres://haohao:haohao@127.0.0.1:5432/haohao?sslmode=disable" up

db-down:
	migrate -path db/migrations -database "postgres://haohao:haohao@127.0.0.1:5432/haohao?sslmode=disable" down 1

db-schema:
	docker compose exec -T postgres pg_dump -U haohao --schema-only --no-owner --no-privileges haohao > db/schema.sql
...
今回は login を一番早く動かしたいので、**最初だけデモユーザーを migration に入れます**。
...
ここで `db/schema.sql` が作られます。このファイルは **手で編集しません**。migration を正本にし、`schema.sql` はレビューと `sqlc` 用のスナップショットにします。
...
`users` テーブルと `pgcrypto` extension が見えていれば大丈夫です。
...
5. `sqlc generate` を実行する
6. `backend/internal/service/*` を増やす
7. `backend/internal/api/*` の Huma operation を増やす
8. `make openapi` で OpenAPI を再生成する
9. `make gen` で frontend SDK も更新する
10. `frontend/src/stores/*` と `frontend/src/views/*` をつなぐ
